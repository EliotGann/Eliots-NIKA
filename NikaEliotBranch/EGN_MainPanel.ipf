...
//	RIGK		RIGAKU R-AXIS : Riguka image plate scanner format
	...
	for(i=0;i<itemsInList(ListOfStringsGB);i+=1)	
		EG_N2G_CreateItem("string",StringFromList(i,ListOfStringsGB))
	endfor	
	...
	if(UseSectors)		//this is all needed for sector analysis. Will need to move stuff around for line analysis later. 
		...
		if (DoSectorAverages)
			...
			For(i=0;i<NumberOfSectors;i+=1)
				ListOfOrientations+=ReplaceString(".",num2str(EG_N2G_roundDecimalPlaces(SectorsStartAngle+SectorsStepInAngle*i,1)),"p")+"_"+ReplaceString(".",num2str(EG_N2G_roundDecimalPlaces(SectorsHalfWidth,1)),"p")+";"
			endfor
...
//		nvar supexchar = root:Packages:EGNika101:SupExChar
//		if(supexchar)
...
Function EGNA_SaveDataPerUserReq(CurOrient,wavelengths)
	...
	if (Use2DdataName)
		...
		if(strlen(UserFileName)<1)	//user did nto set the file name
			...
			if(exists("root:Packages:EGNika101:SupExChar"))
				nvar supexchar = root:Packages:EGNika101:SupExChar
				if(supexchar)
