...
//*******************************************************************************************************************************************
//*******************************************************************************************************************************************
Function EGNM_DisplayMaskOnImage()
...
Function EGNM_ChangeMaskColor(ColorToUse) //red, blue, green
	string ColorToUse
...
//*******************************************************************************************************************************************
//*******************************************************************************************************************************************
Function EGNM_RemoveMaskFromImage()
	DoWindow CCDImageToConvertFig
	if(V_Flag)
...
Function EGNA_SetVarProcMainPanel(ctrlName,varNum,varStr,varName) : SetVariableControl
	...
	if(stringMatch("LineProf_DistanceFromCenter",ctrlName)||stringMatch("LineProf_Width",ctrlName)||stringMatch("LineProf_LineAzAngle",ctrlName)||stringMatch("LineProf_GIIncAngle",ctrlName)||stringMatch("LineProf_EllipseAR",ctrlName))
		...
		EGNA_DrawCenterIn2DGraph()
		EGNA_DrawLinesIn2DGraph()
		EGNA_DrawSectorsIn2DGraph()
...
Function EGNA_LineProfUpdateQ()
		...
		if(stringMatch(LineProf_CurveType,"Horizontal Line") || stringMatch(LineProf_CurveType,"GI_Horizontal line"))
			distance=EGNT_TiltedToCorrectedR( LineProf_DistanceFromCenter*PixelSizeY ,SampleToCCDDistance,VerticalTilt)		//in mm 
			distancew1=EGNT_TiltedToCorrectedR( (LineProf_DistanceFromCenter+LineProf_Width)*PixelSizeY ,SampleToCCDDistance,VerticalTilt)		//in mm 
			distancew2=EGNT_TiltedToCorrectedR(  (LineProf_DistanceFromCenter-LineProf_Width)*PixelSizeY ,SampleToCCDDistance,VerticalTilt)		//in mm 
		endif
		if(stringMatch(LineProf_CurveType,"Vertical Line")|| stringMatch(LineProf_CurveType,"Ellipse")|| stringMatch(LineProf_CurveType,"Angle Line"))
			distance=EGNT_TiltedToCorrectedR( LineProf_DistanceFromCenter*PixelSizeX ,SampleToCCDDistance,HorizontalTilt)		//in mm 
			distancew1=EGNT_TiltedToCorrectedR(  (LineProf_DistanceFromCenter+LineProf_Width)*PixelSizeX ,SampleToCCDDistance,HorizontalTilt)		//in mm 
			distancew2=EGNT_TiltedToCorrectedR(  (LineProf_DistanceFromCenter-LineProf_Width)*PixelSizeX ,SampleToCCDDistance,HorizontalTilt)		//in mm 
		endif
		...
		if( stringMatch(LineProf_CurveType,"GI_Vertical line"))
			Qval = EGNGI_CalculateQxyz(LineProf_DistanceFromCenter+BeamCenterX,BeamCenterY,"xy") // changing from y to xy pure
			Qvalw1 = EGNGI_CalculateQxyz(LineProf_DistanceFromCenter+BeamCenterX+LineProf_Width,BeamCenterY,"xy")
			Qvalw2 = EGNGI_CalculateQxyz(LineProf_DistanceFromCenter+BeamCenterX-LineProf_Width,BeamCenterY,"xy")
		endif
		if( stringMatch(LineProf_CurveType,"GI_Horizontal line"))
			Qval = EGNGI_CalculateQxyz(BeamCenterX,BeamCenterY-LineProf_DistanceFromCenter,"z") // changing to z pure
			Qvalw1 = EGNGI_CalculateQxyz(BeamCenterX,BeamCenterY-LineProf_Width-LineProf_DistanceFromCenter,"Z")
			Qvalw2 = EGNGI_CalculateQxyz(BeamCenterX,BeamCenterY+LineProf_Width-LineProf_DistanceFromCenter,"Z")
		endif
...
Function EGNA_CheckProc(ctrlName,checked) : CheckBoxControl
	...
	if(StringMatch("DisplayQValsOnImage",ctrlName))
		if(checked)
			...
			EGNG_AddQAxisToImage(0)
		else
			EGNG_RemoveQAxisToImage(1)
		endif
	endif
...
//*******************************************************************************************************************************************
//*******************************************************************************************************************************************
Function EGNA_DrawSectorsIn2DGraph()
...
//*******************************************************************************************************************************************
//*******************************************************************************************************************************************
Function EGNA_DrawLinesIn2DGraph()
		...
		if(UseLineProfile)
			...
			if(stringMatch(LineProf_CurveType,"GISAXS_FixQy"))
				isStraightLine=0
					...
					WaveYL=p
					WaveYR=p
					variable Qy0=EGNGI_CalculateQxyz(LineProf_DistanceFromCenter-xcenter,ycenter,"Y")
					WaveX=NIGI_CalcYdimForFixQz(WaveY[p],Qy0)
					Qy0=EGNGI_CalculateQxyz(LineProf_DistanceFromCenter+LineProf_Width-xcenter,ycenter,"Y")
...
Function EGNGI_CalculateQxyz(DimXpos,DimYpos,WhichOne)
...
Function NIGI_CalcYdimForFixQz(DimYPos,Qy)
	...
	variable Qz=EGNGI_CalculateQxyz(0,DimYpos,"Z")
	variable K0val=2*pi/wavelength
...
Function EGNA_DrawCenterIn2DGraph()
	string oldDf=GetDataFOlder(1)
	setDataFolder root:Packages:Convert2Dto1D
 	DoWindow CCDImageToConvertFig
	NVAR displaybeamcenterin2Dgraph=root:Packages:Convert2Dto1D:displaybeamcenterin2Dgraph
...
Function EGNG_AddQAxisToImage(UseGrids)
	variable UseGrids
	...
	SetWindow CCDImageToConvertFig, hook(MyKillGraphHook) = EGNU_KillWindowHookF
...
Function EGNG_RemoveQAxisToImage(Recreate)
	variable Recreate
...
//************************************************************************
//************************************************************************
Function EGNU_UpdateQAxisInImage()
	DoWIndow CCDImageToConvertFig
	if(!V_flag)
		abort
	endif
...
Function EGNU_KillWindowHookF(s)
...
Function TransAx_CalculateVerticalQaxis(w, x)
	...
	Variable x		//in pixels
		...
		if(usegrazingincidence)
			...
			//distinq = EGNGI_CalculateQxyz(BeamCentery,PixPosition,"z") //4*pi*sin((x-beamcentery)*PixelSizey/(2*SampleToCCDDistance))/Wavelength
		elseif(UseLineProfile && (stringMatch(LineProf_CurveType,"GI_Vertical Line") || stringMatch(LineProf_CurveType,"GI_Horizontal Line")))
			...
			DistInQ=EGNGI_CalculateQxyz(BeamCenterX,PixPosition,"Z")
		else
...
//print x
		...
		if(usegrazingincidence)
			//wave data = root:Packages:Convert2Dto1D:ccdimagetoconvert
			//distinq = EGNGI_CalculateQxyz(BeamCenterX,PixPosition,"xy") 
			distinq = 4*pi*sin(atan((x-beamcenterx)*PixelSizeX/(SampleToCCDDistance))/2)/Wavelength
			//dimdelta(data,0) * (x-beamcenterx)
		elseif(UseLineProfile && (stringMatch(LineProf_CurveType,"GI_Vertical Line") || stringMatch(LineProf_CurveType,"GI_Horizontal Line")))
			// this is exception,  need to use GI geometry for conversion, All other should eb the same...
			DistInQ=EGNGI_CalculateQxyz(PixPosition,BeamCenterY,"xypure") // Eliot changing this to xypure rather than "Y"
		else
